...
-- This HNodes instance and W_GTem datatype were copied from the "-ddump-splices" TH output.
-- GHC 9.0.1 won't accept it when generated by TH. Bug?
instance HNodes v => HNodes (GTerm v) where
    type HNodesConstraint (GTerm v) c = (HNodesConstraint v c, c (GTerm v))
    type HWitnessType (GTerm v) = W_GTerm v
    {-# INLINE hLiftConstraint #-}
    hLiftConstraint (HWitness W_GTerm_GTerm_v) = \_ r -> r
    hLiftConstraint (HWitness (E_GTerm_v w)) = hLiftConstraint w
data W_GTerm v n where
    W_GTerm_GTerm_v :: W_GTerm v (GTerm v)
    E_GTerm_v :: HWitness v n -> W_GTerm v n

makeHFunctor ''GTerm
makeHTraversableAndFoldable ''GTerm
