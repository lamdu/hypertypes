-- | A variant of 'Foldable' for 'Hyper.Knot.Knot's
...
-- | 'KFoldable' variant for 'foldMap' for 'Hyper.Knot.Knot's with a single node type (avoids using @RankNTypes@)
{-# INLINE foldMapK1 #-}
foldMapK1 ::
    forall a k n p.
...
-- | 'KFoldable' variant of 'Data.Foldable.traverse_' for 'Hyper.Knot.Knot's with a single node type (avoids using @RankNTypes@)
{-# INLINE traverseK1_ #-}
traverseK1_ ::
    forall f k n p.
    ( Applicative f, KFoldable k
